...
@Quantity int
AS
	INSERT INTO tbl_product(name,brand,price,country,description,material,categoryID,quantity)
	VALUES (@Name,@Brand,@Price,@Country,@Description,@Material,@CategoryId,@Quantity)
GO
--Update product information	
CREATE PROCEDURE UpdateProduct
@Id int, @Name nvarchar(50), @Brand nvarchar(50), @Price money, @Country nvarchar(50), @Description nvarchar(MAX),
@Material nvarchar(50), @CategoryId int, @Quantity int
AS
	UPDATE [tbl_product]
	SET	tbl_product.name = @Name, tbl_product.brand = @Brand, tbl_product.country = @Country, tbl_product.price = @Price, 
		tbl_product.description = @Description, tbl_product.material = @Material, tbl_product.categoryID = @CategoryId, tbl_product.quantity = @Quantity
...
@Role int
AS
	INSERT INTO tbl_user(userId, password, email, fullname, address, phone, gender, role)
	VALUES (@UserId, @Password, @Email, @Fullname, @Address, @Phone, @Gender)
GO
